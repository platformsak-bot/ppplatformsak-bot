...
📞 الواتساب: @kkllaaaaanndbals`, researchKeyboard);
});
...
📞 للاستفسار: @kkllaaaaanndbals`, cvKeyboard);
});
...
📧 alslahyamr1@gmail.com
📞 @kkllaaaaanndbals`, projectKeyboard);
});
...
bot.on('message', (msg) => {
  ...
  if (waitingForAssignment[chatId]) {
    if (photo) {
      bot.sendPhoto('@kkllaaaaanndbals', photo[photo.length - 1].file_id, {
        caption: `📝 واجب جديد\nرقم التواصل: ${chatId}\n\n${messageText}`
      });
    } else {
...
function sendToAdmin(chatId, message) {
  try {
    bot.sendMessage('@kkllaaaaanndbals', message);
...
console.log('📞 للتواصل: @kkllaaaaanndbals');
console.log('📧 البريد: alslahyamr1@gmail.com');

try {
  bot.sendMessage('@kkllaaaaanndbals', '✅ البوت يعمل الآن وجاهز لاستقبال الطلبات!');
} catch (error) {
